...
#     Rcode/       copies of R/*.R and inst/reproduce/*.R
...
# Remove old R files so that deleted functions do not linger
old_files <- list.files(rcode_dir, pattern = "\\.R$", full.names = TRUE)
...
ok <- file.copy(src_files, rcode_dir, overwrite = TRUE)
if (!all(ok)) stop("Failed to copy: ", paste(src_files[!ok], collapse = ", "))

# Confirm that every copy is identical to its source
dst_files <- file.path(rcode_dir, basename(src_files))
same <- unname(tools::md5sum(src_files)) == unname(tools::md5sum(dst_files))
if (!all(same)) {
  stop("Copies differ from sources: ",
       paste(basename(src_files)[!same], collapse = ", "))
}
...
message(sprintf("Supplement written to %s (%d R files, all identical).",
